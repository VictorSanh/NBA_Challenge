...
def preprocess(data_X_train, data_Y_train, portion_train=0.7, main_feature=False) :

	if main_feature :
		# Only the four main features are considered : miss, assist, score, offensive rebound
		filter_col = [col for col in data_X_train if col.startswith('miss') or col.startswith('offensive rebound') or col.startswith('score') or col.startswith('assist')]
		data_X_train = data_X_train[filter_col]
		X = data_X_train.as_matrix()		
	else :
		# Only remove ID
